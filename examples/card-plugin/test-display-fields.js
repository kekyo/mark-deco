...
async function testDisplayFields() {
  console.log('Metadata Field Display Order Control Feature Test\n' + '='.repeat(60));
  ...
  try {
    const resultCustomOrder = await processorCustomOrder.process(markdownFull, "id");
  ...
  try {
    const resultReverseOrder = await processorReverseOrder.process(markdownFull, "id");
  ...
  try {
    const resultOembedDefault = await processorOembedDefault.process(markdownOembed, "id");
  ...
  console.log('• ✅ Card Plugin metadata field display order control with numbers');
  console.log('• ✅ oEmbed Plugin metadata field display order control with numbers');
