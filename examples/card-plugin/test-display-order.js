import { createMarkdownProcessor, createCardPlugin, createOEmbedPlugin, defaultProviderList } from '../../dist/index.js';
...
async function testDisplayOrder() {
  ...
  try {
    ...
    console.log(`Element positions: image=${imageIndex}, description=${descriptionIndex}, title=${titleIndex}`);
    
    if (imageIndex < titleIndex && descriptionIndex < titleIndex) {
      console.log('✓ Display order is correct (image and description before title)');
    } else {
  ...
  try {
    const resultOembed = await processorOembedReverse.process(markdownOembed, "id");
    ...
    console.log(`Element positions: link=${linkIndex}, provider=${providerIndex}, title=${titleIndex}`);
    
    if (linkIndex !== -1 && providerIndex !== -1 && titleIndex !== -1) {
      if (linkIndex < providerIndex && providerIndex < titleIndex) {
        console.log('✓ Reverse order is correct (external link, provider, then title)');
      } else {
  ...
  try {
    const resultEnhanced = await processorEnhanced.process(markdownAmazon, "id");
    ...
    if (html.includes('card-enhanced-info')) {
      ...
      console.log('ℹ️ Enhanced information not available (expected with example.com)');
    }
  ...
  try {
    ...
    console.log(`Default fields present: title=${hasTitle}, image=${hasImage}, description=${hasDescription}, provider=${hasProvider}, link=${hasLink}`);
