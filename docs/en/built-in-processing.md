...
| `beautiful-mermaid` | Renders Mermaid diagrams to SVG or ASCII using `beautiful-mermaid`      |
...
You can set a default `target` (and optional `rel`) for all Markdown links generated by the processor:

```typescript
const result = await processor.process(markdown, 'id', {
  linkTarget: '_blank',
  // Optional: override the default rel value
...
The Beautiful Mermaid plugin renders Mermaid diagrams to SVG or ASCII at build time using
[beautiful-mermaid](https://github.com/lukilabs/beautiful-mermaid). It uses the same
```mermaid``` code block syntax but does not require Mermaid.js on the client side.
...
// Generate Beautiful Mermaid plugin (SVG output by default)
...
Output HTML uses `beautiful-mermaid-*` CSS classes on the outer element:

```html
<div class="beautiful-mermaid-wrapper beautiful-mermaid-svg" id="id-1">
  <!-- SVG content generated by beautiful-mermaid -->
</div>
```

ASCII output is also supported:
