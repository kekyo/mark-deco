...
You can offset output heading levels with `headingBaseLevel`:
...
With `headingBaseLevel: 2`, `#` becomes `<h2>`, `##` becomes `<h3>`, and so on (clamped at `<h6>`).
The `headingTree.level` values follow the adjusted levels.
